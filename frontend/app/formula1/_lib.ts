import { Formula1Article, StrapiFormula1Article } from "./types";
...
export function getTimeAgo(dateString: string): string {
  ...
  export function getImageUrl(imageUrl: string | undefined): string {
    if (!imageUrl) return '/f1.png'; // Default fallback
   ...
   */
  export function transformArticle(article: StrapiFormula1Article): Formula1Article {
    return {
      id: article.id,
      title: article.title,
